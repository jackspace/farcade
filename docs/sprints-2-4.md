...
| 15.3 | Renders: standings as HTML page + NomadNet page + plain-text digest (email-ready); each player shows rating AND **completion rate** (finished games vs policy-adjudicated timeouts, old marks decaying — mesh life means silence is sometimes a mountain, so it reads as reliability, not a scarlet letter; Jack 2026-08-10) | A stranger can read the league table without any Farcade software; completion rate reproducible from the same cert+claim history by any keeper | sonnet | M |
